...
class GPTNeoSelfAttention(nn.Module):
    ...
    def forward(
        ...
        print(f"query_states: {query.shape}, key_states: {key.shape}, value_states: {value.shape}")
        save_dir = get_path()
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        torch.save(query, os.path.join(save_dir, f"GPTNEO_key_vector_layer1_heatmap.pt"))
        torch.save(key, os.path.join(save_dir, f"GPTNEO_query_vector_layer1_heatmap.pt"))
        torch.save(value, os.path.join(save_dir, f"GPTNEO_value_vector_layer1_heatmap.pt"))
        exit()
